...
var ip_check_data = [
    ...
    [true ,"1:2::7:8"],
    [true ,"1::7:8"],
    [false ,"1:2:3:4:5:6:1.2.3.4"],
    [false ,"1:2:3:4:5::1.2.3.4"],
    [false ,"1:2:3:4::1.2.3.4"],
    [false ,"1:2:3::1.2.3.4"],
    [false ,"1:2::1.2.3.4"],
    [false ,"1::1.2.3.4"],
    [false ,"1:2:3:4::5:1.2.3.4"],
    [false ,"1:2:3::5:1.2.3.4"],
    [false ,"1:2::5:1.2.3.4"],
    [false ,"1::5:1.2.3.4"],
    [false ,"1::5:11.22.33.44"],
    [false,"1::5:400.2.3.4"], 
    [false,"1::5:260.2.3.4"], 
    [false,"1::5:256.2.3.4"], 
    [false,"1::5:1.256.3.4"], 
    ...
    [false,"::3000.30.30.30"], 
    [false ,"fe80::217:f2ff:254.7.237.98"],
...
exports.get_ipany_re = {
    /* jshint: maxlen: false */
    'check if real IPs': function (test) {
        /* for x-*-ip headers */
        test.expect(139);
        var bare_re = net_utils.get_ipany_re();
        for(var i in ip_check_data ){
            var result = bare_re.test(ip_check_data[i][1]);
            test.ok( (result === ip_check_data[i][0]), (result === ip_check_data[i][0]) + '  ->  ' + ip_check_data[i][1]);
        }
        test.done();
    },
    'IPv4, single, bracketed': function (test) {
        /* for x-*-ip headers */
        test.expect(2);
        var bare_re = net_utils.get_ipany_re();
        var match = bare_re.exec('[127.0.0.1]');
        // console.log(match);
        test.equal(match[1], '127.0.0.1');
        test.equal(match.length, 2);
        test.done();
    },
    'IPv4, Received header, parens, single': function (test) {
        test.expect(2);
    ...
    'IPv4, Received header, bracketed, single': function (test) {
        test.expect(2);
    ...
    'IPv4, Received header, bracketed, single, 2': function (test) {
        test.expect(2);
        var received_re = net_utils.get_ipany_re('^Received:.*?[\\[\\(]', '[\\]\\)]');
    ...
    'IPv6, Received header, bracketed, single': function (test) {
        test.expect(2);
