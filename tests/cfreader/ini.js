...
exports.load = {
    ...
    'returns a template ini for non-existing files': function(test) {
    	test.expect(1);
    	var result = this.ini.load('tests/config/non-existent.ini');
    ...
    'non-exist.ini is template' : function (test) {
        test.expect(1);
        test.deepEqual(this.ini.load('non-exist.ini'), { main: { } } );
        test.done();
    },
    'non-exist.ini boolean' : function (test) {
        test.expect(1);
        test.deepEqual(
                this.ini.load('non-exist.ini',
                    { booleans: ['reject']}),
                { main: { reject: false } }
                );
        test.done();
    ...
    'test.ini, opts' : function (test) {
        test.expect(4);
        var r = this.ini.load('tests/config/test.ini', testopts, regex).main;
