...
class HMailItem extends events.EventEmitter {
    ...
    found_mx (err, mxs) {
        ...
        if (err) {
            ...
            for (const mx in mxlist) {
                ...
                else if (cfgmod.cfg.ipv6_enabled) {
                    this.mxlist.push(
    ...
    try_deliver_host_on_socket (mx, host, port, socket) {
        ...
        function process_ehlo_data () {
            ...
            if (!cfgmod.cfg.enable_tls) return auth_and_mail_phase();      // TLS not enabled
        ...
        function finish_processing_mail (success) {
            ...
            if (cfgmod.cfg.pool_concurrency_max && success) {
                client_pool.release_client(socket, port, host, mx.bind, fin_sent);
            }
            else if (cfgmod.cfg.pool_concurrency_max && !mx.using_lmtp) {
                send_command('RSET');
            }
            else {
                send_command('QUIT');
        ...
        socket.on('line', line => {
            ...
            switch (command) {
                ...
                case 'dot_lmtp':
                    ...
                    break;
                case 'quit':
                    if (cfgmod.cfg.pool_concurrency_max) {
    ...
    temp_fail (err, extra) {
        ...
        if (this.num_failures >= (cfgmod.cfg.maxTempFailures)) {
