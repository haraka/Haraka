...
function createServer(options, cb) {
    var serv = net.createServer(function (cryptoSocket) {
        ...
        if(debug)console.log("new ps");
        var socket = new pluggableStream(cryptoSocket);
        socket.cryptoSocket = cryptoSocket;
        socket.cryptoOptions= options; // Save the options so can access them later.

        socket.upgrade = function (options) {
            if(debug)console.log("Calling upgrade!!");
            socket.clean();
...
function connect(port /* host, options, cb */) {
    ...
    socket.upgrade = function (options) {
        ...
        if(debug)console.log("called upgrade awaiting secured.");
    };
