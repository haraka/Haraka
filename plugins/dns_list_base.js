...
exports.lookup = function (lookup, zone, cb) {
    ...
    dns.resolve(query, 'A', (err, a) => {
        ...
        a.find((rec) => rec.split('.')[0] !== '127')) || a?.includes('127.255.255.')
        ) {
...
exports.first = function (lookup, zones, cb, cb_each) {
    ...
    this.multi(lookup, zones, (err, zone, a, pending) => {
        ...
        if (ran_cb || pending && (err || !a)) return;
...
exports.disable_zone = function (zone, result) {
    if (!zone || !this.zones || !this.zones.length || !this.disable_allowed) return false;
    ...
    this.logwarn(`disabling zone '${zone}'${result ? strResult : ''}`);
    return true;
}
