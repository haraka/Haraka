...
exports.load_config = function () {
    ...
    plugin.cfg = plugin.config.get('data.greylist.ini', {
        booleans : [
            '+skip.dnswlorg',
            '-skip.mailspikewl'
        ]
        ...
        for (var i = 0; i < list.length; i++) {
            try {
                ...
                if (addr.match(/\/\d+$/)) {
                    ...
                    addr = [ipaddr.parse(addr), ((isIPv6(addr)) ? 128 : 32)];
                }
...
exports.hook_rcpt_ok = function (next, connection, rcpt) {
    ...
    plugin.check_and_update_white(connection, function (err, white_rec) {
        ...
        if (white_rec) {
            ...
            if (plugin.is_sender_special(mail_from)) { // postponed till DATA
                ctr.add(plugin, {
                    skip : 'special-sender',
                    stage : 'rcpt'
                });
...
exports.hook_data = function (next, connection) {
    ...
    if (!ctr.has(plugin, 'skip', 'special-sender')){
        return next();
    }
    ...
    return async.detect(connection.transaction.rcpt_to, function (rcpt, cb) {
        plugin.logdebug(connection, 'Special Proceeding:' + rcpt);
        return plugin.process_tuple(connection, from_addr, rcpt.address(), function (err, val) {
            return cb(null, val);
        });
    }, function (white_promo_rec) {
