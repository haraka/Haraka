...
SendClient.prototype.release = function () {
    ...
    if (this.sent === control.getEspConfig(this.dom, 'sessions')) {
	this.destroy();
	return;
    }
...
exports.run_send = function(conn_pool, dom, port, host, timeout, enable_tls,
	...
	var closed = function (msg) {
		...
		if (send_client.state === STATE_ACTIVE) {
		    ...
		    hmail.not_send(send_client.bad_code | send_client.data_sent
                                   | send_client.error);
                    ...
                    // we did nothing in not_send(); in the latter case, call
                    // temp_fail in not_send; destroy the client
                    if (send_client.state === STATE_IDLE) {
			send_client.state = STATE_DEAD;                        
			hmail.not_send(send_client.bad_code |
	...
	send_client.on('helo', function () {
        ...
        send_client.on('error', function (msg) {
            send_client.error = constants.error;
            hmail.not_send(constants.error);
            send_client.destroy();
        });
